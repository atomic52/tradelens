...
export default function TradeDetail() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <button onClick={() => navigate("/trades")} className="text-sm text-blue-600 hover:underline">
        ← Back to trades
      </button>
      ...
      <div className="bg-white rounded-lg border p-6 space-y-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5 capitalize">{trade.asset_class} · {trade.direction}</p>
          </div>
          <span
            className={clsx(
              "text-xl font-semibold tabular-nums",
              pnl == null ? "text-gray-400" : pnl >= 0 ? "text-green-600" : "text-red-500"
            )}
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm border-t pt-4">
          {[
            ...
            ["Status", <span className={clsx("capitalize", trade.status === "open" ? "text-amber-600" : "text-green-600")}>{trade.status}</span>],
